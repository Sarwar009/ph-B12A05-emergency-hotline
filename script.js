...
getId('main').addEventListener('click', e => {
  ...
  if (e.target.classList.contains('copy-btn')) {
    ...
    const number = e.target.closest('.btn-parent-div').querySelector('p:nth-of-type(2)').innerText;
    navigator.clipboard.writeText(number).then(() => {
      alert(`Copied: ${number}`);
    });
  }
  ...
  if (e.target.classList.contains('call-btn')) {
    ...
    if (current >= 20) {
      ...
      const data = {
        category: e.target.closest('.btn-parent-div').querySelector('p:nth-of-type(3)').innerText,
        number: e.target.closest('.btn-parent-div').querySelector('p:nth-of-type(2)').innerText,
...
function renderHistory() {
  ...
  historyData.forEach(item => {
    const div = document.createElement('div');
    div.className = 'rounded-lg bg-[#FAFAFA] p-3 mb-3 flex justify-between items-center';
    div.innerHTML = `
      <div>
        <h3 class="text-black font-bold">${item.category}</h3>
        <p>${item.number}</p>
